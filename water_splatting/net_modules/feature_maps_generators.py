...
class Unet_model(nn.Module):
    def __init__(self,features_dim=32,backbone="resnet18",use_features_mask=False,use_independent_mask_branch=False):
        ...
        if use_features_mask:
            ...
            #     nn.ConvTranspose2d(backbone_channels[-1], backbone_channels[-2], kernel_size=3, stride=2),
            #     OneConv(backbone_channels[-2],backbone_channels[-2]),
            #     nn.ConvTranspose2d(backbone_channels[-2], backbone_channels[-3], kernel_size=3, stride=2),
            #     OneConv(backbone_channels[-3],backbone_channels[-3]),
            #     nn.ConvTranspose2d(backbone_channels[-3], backbone_channels[-4], kernel_size=3, stride=2),
            ...
            self.mask_decoder=nn.Sequential(
                PixelShuffleBlock(backbone_channels[-1], backbone_channels[-2]),
                OneConv(backbone_channels[-2],backbone_channels[-2]),
                PixelShuffleBlock(backbone_channels[-2], backbone_channels[-3]),
                OneConv(backbone_channels[-3],backbone_channels[-3]),
                PixelShuffleBlock(backbone_channels[-3], backbone_channels[-4]),
                OneConv(backbone_channels[-4],backbone_channels[-4]),
                nn.Conv2d(backbone_channels[-4], 1, kernel_size=1),
                nn.Sigmoid()
            )
    ...
    def forward(self, inx,only_gloabal_features=False,eval_mode=False):
        ...
        # print("Input Shape: ", inx.shape)
        # print("Features Shape: ", x.shape)

        # if torch.randint(0, 100, (1,)).item() == 22:
        #     img_num = torch.randint(0, 1000, (1,)).item()
        #     torchvision.utils.save_image(x[0, 0].repeat(3,1,1), os.path.join("features", f"feature512_{img_num}.png"))
        #     print("Saved Features no.", img_num)
        #     torch.save(x, os.path.join("features", f"feature512_{img_num}.pt"))

        # print("std:", x.std().item())
        # print("unique approx:", torch.unique(x.round(decimals=3)).shape)
